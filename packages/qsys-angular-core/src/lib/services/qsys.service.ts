...
export class QSysService {
  ...
  private config: QSysConfig;

  constructor(
    private injector: Injector,
    @Optional() @Inject(QSYS_CONFIG) config?: QSysConfig
  ) {
    this.config = { ...DEFAULT_QSYS_CONFIG, ...config } as QSysConfig;
  }

  /**
   * Get the HTTP API URL based on config
   */
  private getHttpApiUrl(): string {
    const port = this.config.corePort || 9091;
    return `http://${this.config.coreIp}:${port}/api`;
  }
  ...
  private async getComponentControlsViaHTTP(componentName: string): Promise<any[]> {
    const url = `${this.getHttpApiUrl()}/components/${encodeURIComponent(componentName)}/controls`;
  ...
  async setControlViaHTTP(componentName: string, controlName: string, value: any): Promise<void> {
    const url = `${this.getHttpApiUrl()}/components/${encodeURIComponent(componentName)}/controls/${encodeURIComponent(controlName)}`;
