...
export class AuthService {
  ...
  constructor(@Optional() @Inject(QSYS_CONFIG) config?: QSysConfig) {
    this.config = { ...DEFAULT_QSYS_CONFIG, ...config } as QSysConfig;

    // Set defaults from config
    this.username.set(this.config.defaultUsername || 'admin');
    this.password.set(this.config.defaultPassword || 'admin');

    this.loadCredentialsFromStorage();
  }

  /**
   * Get the Core IP from config
   */
  getCoreIp(): string {
    return this.config.coreIp;
  }
  ...
  async login(username: string, password: string): Promise<void> {
    ...
    try {
      const coreIp = this.config.coreIp;
